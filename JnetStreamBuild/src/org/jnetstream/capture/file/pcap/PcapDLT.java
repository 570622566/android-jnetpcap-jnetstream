...
public enum PcapDLT {
	...
	public Protocol asProtocol() {
		return ProtocolRegistry.translate(PcapFile.class, intValue);
	}
	...
	public static PcapDLT asPcap(Protocol p) {
		...
		int raw = ProtocolRegistry.translate(PcapFile.class, p);
		
		for (PcapDLT pcap : values()) {
			if (pcap.intValue == raw) {
				return pcap;
			}
		}
  ...
  public static Protocol asConst(long rawLinkType) {
  	...
  	return ProtocolRegistry.translate(PcapFile.class, (int) rawLinkType);
  }
